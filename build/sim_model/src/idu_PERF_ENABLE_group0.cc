#include <sdp.h>
bool sdp::decode_sdp_PERF_ENABLE_group0() {
...
auto& univ_var_87 = local_var_24;
return univ_var_87;
}
void sdp::update_sdp_PERF_ENABLE_group0() {
...
sdp_group0_d_perf_dma_en = local_var_1_nxt_holder;
sdp_group0_d_perf_lut_en = local_var_2_nxt_holder;
sdp_group0_d_perf_nan_inf_count_en = local_var_3_nxt_holder;
sdp_group0_d_perf_sat_en = local_var_4_nxt_holder;
#ifdef ILATOR_VERBOSE
instr_update_log << "No." << std::dec << GetInstrCntr() << '\t' << "PERF_ENABLE_group0 state updates:" << std::endl;
instr_update_log << "    sdp_group0_d_perf_dma_en => " << std::hex << "0x" << sdp_group0_d_perf_dma_en << std::endl; 
instr_update_log << "    sdp_group0_d_perf_lut_en => " << std::hex << "0x" << sdp_group0_d_perf_lut_en << std::endl; 
instr_update_log << "    sdp_group0_d_perf_nan_inf_count_en => " << std::hex << "0x" << sdp_group0_d_perf_nan_inf_count_en << std::endl; 
instr_update_log << "    sdp_group0_d_perf_sat_en => " << std::hex << "0x" << sdp_group0_d_perf_sat_en << std::endl; 
