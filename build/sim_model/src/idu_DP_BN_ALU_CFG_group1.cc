#include <sdp.h>
bool sdp::decode_sdp_DP_BN_ALU_CFG_group1() {
...
sdp_group1_d_bn_alu_shift_value = local_var_1_nxt_holder;
sdp_group1_d_bn_alu_src = local_var_2_nxt_holder;
#ifdef ILATOR_VERBOSE
instr_update_log << "No." << std::dec << GetInstrCntr() << '\t' << "DP_BN_ALU_CFG_group1 state updates:" << std::endl;
instr_update_log << "    sdp_group1_d_bn_alu_shift_value => " << std::hex << "0x" << sdp_group1_d_bn_alu_shift_value << std::endl; 
instr_update_log << "    sdp_group1_d_bn_alu_src => " << std::hex << "0x" << sdp_group1_d_bn_alu_src << std::endl; 
