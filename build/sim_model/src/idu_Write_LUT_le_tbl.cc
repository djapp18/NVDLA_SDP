#include <sdp.h>
bool sdp::decode_sdp_Write_LUT_le_tbl() {
...
auto& univ_var_11633 = local_var_8;
return univ_var_11633;
}
void sdp::update_sdp_Write_LUT_le_tbl() {
std::map<sc_biguint<7>, sc_biguint<16>> local_var_0;
store_11634(local_var_0);
for (auto& it : local_var_0) {
  sdp_le_tbl[it.first] = it.second;
}
#ifdef ILATOR_VERBOSE
instr_update_log << "No." << std::dec << GetInstrCntr() << '\t' << "Write_LUT_le_tbl state updates:" << std::endl;
