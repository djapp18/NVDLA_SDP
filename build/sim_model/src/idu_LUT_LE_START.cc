#include <sdp.h>
bool sdp::decode_sdp_LUT_LE_START() {
...
auto& univ_var_7 = local_var_17;
return univ_var_7;
}
void sdp::update_sdp_LUT_LE_START() {
auto local_var_1 = sdp_csb_data.range(31, 0);
auto local_var_1_nxt_holder = local_var_1;
sdp_s_lut_le_start = local_var_1_nxt_holder;
#ifdef ILATOR_VERBOSE
instr_update_log << "No." << std::dec << GetInstrCntr() << '\t' << "LUT_LE_START state updates:" << std::endl;
instr_update_log << "    sdp_s_lut_le_start => " << std::hex << "0x" << sdp_s_lut_le_start << std::endl; 
