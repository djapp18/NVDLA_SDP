#include <sdp.h>
bool sdp::decode_sdp_DP_BS_CFG_group1() {
...
auto& univ_var_32 = local_var_25;
return univ_var_32;
}
void sdp::update_sdp_DP_BS_CFG_group1() {
...
sdp_group1_d_bs_alu_algo = local_var_1_nxt_holder;
sdp_group1_d_bs_alu_bypass = local_var_2_nxt_holder;
sdp_group1_d_bs_bypass = local_var_3_nxt_holder;
sdp_group1_d_bs_mul_bypass = local_var_4_nxt_holder;
sdp_group1_d_bs_mul_prelu = local_var_5_nxt_holder;
sdp_group1_d_bs_relu_bypass = local_var_6_nxt_holder;
#ifdef ILATOR_VERBOSE
instr_update_log << "No." << std::dec << GetInstrCntr() << '\t' << "DP_BS_CFG_group1 state updates:" << std::endl;
instr_update_log << "    sdp_group1_d_bs_alu_algo => " << std::hex << "0x" << sdp_group1_d_bs_alu_algo << std::endl; 
instr_update_log << "    sdp_group1_d_bs_alu_bypass => " << std::hex << "0x" << sdp_group1_d_bs_alu_bypass << std::endl; 
instr_update_log << "    sdp_group1_d_bs_bypass => " << std::hex << "0x" << sdp_group1_d_bs_bypass << std::endl; 
instr_update_log << "    sdp_group1_d_bs_mul_bypass => " << std::hex << "0x" << sdp_group1_d_bs_mul_bypass << std::endl; 
instr_update_log << "    sdp_group1_d_bs_mul_prelu => " << std::hex << "0x" << sdp_group1_d_bs_mul_prelu << std::endl; 
instr_update_log << "    sdp_group1_d_bs_relu_bypass => " << std::hex << "0x" << sdp_group1_d_bs_relu_bypass << std::endl; 
