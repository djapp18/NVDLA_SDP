#include <sdp.h>
bool sdp::decode_sdp_Write_LUT_lo_tbl() {
...
auto& univ_var_17750 = local_var_8;
return univ_var_17750;
}
void sdp::update_sdp_Write_LUT_lo_tbl() {
std::map<sc_biguint<9>, sc_biguint<16>> local_var_0;
store_17751(local_var_0);
for (auto& it : local_var_0) {
  sdp_lo_tbl[it.first] = it.second;
}
#ifdef ILATOR_VERBOSE
instr_update_log << "No." << std::dec << GetInstrCntr() << '\t' << "Write_LUT_lo_tbl state updates:" << std::endl;
