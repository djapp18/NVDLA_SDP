#include <sdp.h>
bool sdp::decode_sdp_LUT_LO_END() {
...
auto& univ_var_10 = local_var_17;
return univ_var_10;
}
void sdp::update_sdp_LUT_LO_END() {
auto local_var_1 = sdp_csb_data.range(31, 0);
auto local_var_1_nxt_holder = local_var_1;
sdp_s_lut_lo_end = local_var_1_nxt_holder;
#ifdef ILATOR_VERBOSE
instr_update_log << "No." << std::dec << GetInstrCntr() << '\t' << "LUT_LO_END state updates:" << std::endl;
instr_update_log << "    sdp_s_lut_lo_end => " << std::hex << "0x" << sdp_s_lut_lo_end << std::endl; 
